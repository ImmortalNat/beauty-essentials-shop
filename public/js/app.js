...
function renderCategoryButtons() {
  ...
  filterContainer.innerHTML = `
    ...
    ${categories.map(c => `<button class="filter-btn" onclick="filterCat('${c.name}', this)">${c.icon} ${c.name}</button>`).join('')}
  `;
}

function render(l) {
  const g = document.getElementById('productsGrid'); if(!g)return;
  if (l.length === 0) { g.innerHTML = '<p style="grid-column:1/-1; text-align:center; padding:3rem; color:#6c757d;">No items in this category.</p>'; return; }
  g.innerHTML = l.map(p => `
...
async function load() {
  try {
    ...
    if (contactContainer) {
      contactContainer.innerHTML = `<a href="https://wa.me/${wa.replace(/[^0-9]/g, '')}" target="_blank" class="contact-btn">💬 WhatsApp +${wa.replace(/[^0-9]/g, '')}</a> ${p2 ? `<a href="https://wa.me/233${p2.replace(/[^0-9]/g, '').replace(/^0/,'')}" target="_blank" class="contact-btn">💬 WhatsApp ${p2}</a>` : ''}`;
    }
